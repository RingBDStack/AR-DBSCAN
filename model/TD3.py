...
class Actor(nn.Module):
    ...
    def __init__(self, global_state_dim, local_state_dim, action_dim,
                 max_action, device):
        """
    ...
    def forward(self, global_states, local_states):
        ...
        :param global_states: global states of the environment, for the overall searching and clustering situation.
        :param local_states: local states of the environment for the description of the situation of each cluster.
        :return: max_action * attention coefficient
        """
        # global_states: [1, 1, global_state_dim]
        # local_states: [1, cluster_num, local_state_dim]
        # att
        states = torch.zeros((global_states.shape[0], 64))
        # encode global_states and local states into fixed length states [1, 64]
        for i, global_state, local_state in zip(range(global_states.shape[0]),
                                                global_states, local_states):
            w_global_state = self.W(global_state)  # [1, 32]
            u_local_state = self.U(
                torch.FloatTensor(local_state).to(
                    self.device))  # [cluster_num, 32]
            # u_local_state = self.U(local_state)  # [cluster_num, 32]
            global_ = w_global_state.expand(
                u_local_state.shape[0] + 1,
                w_global_state.shape[1])  # [1+cluster_num, 32]
            local_ = torch.cat(
                (w_global_state, u_local_state))  # [1+cluster_num, 32]

            attention_weight = F.leaky_relu(
                self.att(torch.cat((global_, local_),
                                   dim=1)))  # [1+cluster_num, 1]
            attention_weight = torch.div(attention_weight,
                                         torch.sum(attention_weight))
            # print(w_global_state.shape, u_local_state.shape, global_.shape,
            #       local_.shape)
            # print(attention_weight[0])
            states[i] = F.relu(
                torch.cat(
                    (torch.mul(attention_weight[0], w_global_state),
                     torch.sum(torch.mul(attention_weight[1:], u_local_state),
                               0).reshape(1, -1)),
                    dim=1))  # [1, 64]
        a = F.relu(self.l1(states))  # [1, 256]
        a = F.relu(self.l2(a))  # [1, 256]
        a = torch.sigmoid(self.l3(a))  # [1, action_dim]
...
class Critic(nn.Module):
    ...
    def forward(self, global_states, local_states, actions):
        ...
        for i, global_state, local_state in zip(range(global_states.shape[0]),
            ...
            u_local_state = self.U(
                torch.FloatTensor(local_state).to(self.device))

            global_ = w_global_state.expand(u_local_state.shape[0] + 1,
                                            w_global_state.shape[1])
            local_ = torch.cat((w_global_state, u_local_state))

            global_local_score = F.leaky_relu(
                self.att(torch.cat((global_, local_), 1)))
            global_local_score_ = torch.div(global_local_score,
                                            torch.sum(global_local_score))

            states[i] = F.relu(
                torch.cat((torch.mul(global_local_score_[0], w_global_state),
                           torch.sum(
                               torch.mul(global_local_score_[1:],
                                         u_local_state), 0).reshape(1, -1)),
                          1))
...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        # local_state has varying cluster size, so we choose to return list for it.
        return (torch.FloatTensor([self.global_state[i]
                                   for i in ind]).to(self.device),
                [self.local_state[i] for i in ind],
                torch.FloatTensor(self.action[ind]).to(self.device),
                torch.FloatTensor([self.next_global_state[i]
                                   for i in ind]).to(self.device),
...
class Skylark_TD3():
    ...
    def __init__(self,
        ...
        self.actor = Actor(self.global_state_dim, self.local_state_dim,
                           self.action_dim, self.max_action,
    ...
    def select_action(self, states):
        ...
        """
        global_states = torch.FloatTensor(states[0]).to(
            self.device).unsqueeze(0)
        # local_states = torch.FloatTensor(states[1]).to(
        #     self.device).unsqueeze(0)
        local_states = list([states[1]])
        return self.actor(global_states,
                          local_states).cpu().data.numpy().flatten()
    ...
    def learn(self, replay_buffer, batch_size=16):
        ...
        with torch.no_grad():
            ...
            next_action = (
                self.actor_target(next_global_state, next_local_state) +
                noise).clamp(0, self.max_action)

            # Compute the target Q value
            target_Q1, target_Q2 = self.critic_target(next_global_state,
                                                      next_local_state,
                                                      next_action)
        ...
        if self.total_iteration % self.policy_freq == 0:
            ...
            # Compute actor losse
            actor_loss = -self.critic.Q1(global_state, local_state,
                                         self.actor(global_state,
                                                    local_state)).mean()
