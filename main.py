...
parser.add_argument('--block_num',
                    default=1,
                    type=int,
                    help="The number of data blcoks")
parser.add_argument(
...
def DRL_DBSCAN(args, features, labels, reward_mask, agent_id):
    """
        ...
        with trange(args.layer_num, leave=False) as test_iter:
            for l in test_iter:
                test_iter.set_description(
                    f'Agent {agent_id} Block {b} Test layer {l}')
                agent = agents[l]
        ...
        with trange(args.layer_num) as train_iter:
            for l in train_iter:
                train_iter.set_description(
                    f'Agent {agent_id} Block {b} Train layer {l}')
                if l == 0:
                    max_nmi = -1
                agent = agents[l]
                ...
                episode_iter.desc = f'Agent {agent_id} Block {b} Training Layer {l}'
...
if __name__ == '__main__':
    ...
    for seed in seed_pool:
        ...
        reward_mask, features, labels = load_data_shape(
            f'/datasets/SAR_DBSCAN/{args.dataset}.txt', args.train_size)
        reward_mask, features, labels = [reward_mask], [features], [labels]
        # pre agent find
        se_label, uncertainty, se_label_with_noise = se_pre_partition(
            features[0])
        print('uncertainty: ', uncertainty, flush=True)
        se_nmi, se_ami, se_ari = dbscan_metrics(labels[0], se_label_with_noise)
        ...
        print(se_label_with_noise.max(), se_label.max(), flush=True)
        print('agent number: ', pre_partition.max() + 1, flush=True)
        data = []
        labels_ = []
        reward_mask_ = []
        original_id = torch.arange(features[0].shape[0])
        id_idx_ = []
        if pre_partition.max() == -1:
            data.append(features[0])
            labels_.append(labels[0])
            reward_mask_.append(reward_mask[0])
            id_idx_.append(original_id)
        else:
            for id in range(pre_partition.max() + 1):
                data.append(features[0][se_label == id])
                labels_.append(labels[0][se_label == id])
                id_idx = (se_label == id
                          ).nonzero().squeeze()  # index of points in partition
                id_idx_.append(id_idx)
                combined, count = torch.cat(
                    (id_idx, torch.tensor(reward_mask[0])),
                    dim=0).unique(return_counts=True)
                partition_original_id = original_id[
                    se_label == id]  # original id of partition
                partition_mask = combined[count >
                                          1]  # mask id need to suit partition
        ...
        for agent_id, feature_i, label_i, reward_mask_i in zip(
            ...
            cur_label, cur_cluster_num = DRL_DBSCAN(
                args, feature_i, label_i, reward_mask_i, agent_id)
        ...
        final_label = torch.zeros_like(torch.tensor(labels[0])).scatter_(
            0, id_idx_, final_label)
        final_label = final_label.numpy()

        d_nmi, d_ami, d_ari = dbscan_metrics(labels[0], final_label)
        purity = purity_score(labels[0], final_label)
        nmi_list.append(d_nmi)
        ami_list.append(d_ami)
        ari_list.append(d_ari)
    ...
    print("AMI: ", np.mean(ami_list), np.std(ami_list))
    print("ARI: ", np.mean(ari_list), np.std(ari_list))
    print("Purity: ", np.mean(purity_list), np.std(purity_list))
